...
-- Drop the existing foreign key constraint on resolution_notifications.market_id
ALTER TABLE public.resolution_notifications
  DROP CONSTRAINT resolution_notifications_market_id_fkey;

-- Make market_id nullable so contest notifications can have NULL market_id
ALTER TABLE public.resolution_notifications
  ALTER COLUMN market_id DROP NOT NULL;

-- Add a nullable contest_id column for contest notifications
ALTER TABLE public.resolution_notifications
  ADD COLUMN contest_id uuid REFERENCES public.prediction_contests(id);

-- Re-add the FK but only validate when market_id is not null
ALTER TABLE public.resolution_notifications
  ADD CONSTRAINT resolution_notifications_market_id_fkey
  FOREIGN KEY (market_id) REFERENCES public.markets(id);

-- Update the resolve_contest function to use contest_id instead of market_id
...
BEGIN
  ...
  IF _participant_count < _min_participants THEN
    ...
    LOOP
      ...
      -- Notify about voided contest (use contest_id column, not market_id)
  ...
  LOOP
    ...
    LOOP
      ...
      -- Notify winner (use contest_id column)
      INSERT INTO resolution_notifications (user_id, contest_id, notification_type, title, message, outcome, payout_amount)
